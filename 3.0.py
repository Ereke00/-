...
def get_messages_by_category(category):
    if category == 'Спам':
        messages = mongo.db.messages.find({'category': 0})  # Фильтровать по категории "Спам"
    elif category == 'Вакансии':
        messages = mongo.db.messages.find({'category': 1})  # Фильтровать по категории "Вакансии"
    elif category == 'Проекты':
        messages = mongo.db.messages.find({'category': 2})  # Фильтровать по категории "Проекты"
    elif category == 'Входящие':  
        messages = mongo.db.messages.find()  # Получить все сообщения
    else:
        messages = mongo.db.messages.find({'category': int(category)})  # Фильтровать по другим категориям
